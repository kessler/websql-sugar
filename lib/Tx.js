...
function Tx(db) {
	...
	this._execTransaction = bind(db.transaction, db)
...
Tx.prototype.execute = function(fn) {
	...
	this._execTransaction(
		bind(this._txCallback, this),
		bind(this._txErrorCallback, this),
		bind(this._txSuccessCallback, this)
	)
}
...
Tx.prototype._txCallback = function(tx) {
	...
	for (var i = 0; i < queryCommands.length; i++) {
		...
		var successCallback = bind(this._executeSqlSuccessCallback, this, command)
		var errorCallback = bind(this._executeSqlErrorCallback, this, command)
